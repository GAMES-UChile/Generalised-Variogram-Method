...
class gpinit:
    ...
    def general_loss(self, K, S, metric):
        ...
        elif metric == 'FW2': #not ready
            return np.sqrt(np.sum((qnS2-qnS1)**2))
        elif metric == 'FW1': #not ready
            return np.sum(np.abs(np.cumsum(nS1)-np.cumsum(nS2)))
        elif metric == 'FKL': #not ready
            return np.sum((np.log(nS1)-np.log(nS2))*nS1)
        elif metric == 'FIS': #not ready
            d = np.log(nS1)-np.log(nS2)
            return np.sum( np.exp(d) - d - 1)
            #return np.sum(nS1/nS2 - np.log(nS1) + np.log(nS1) -1  )
    ...
    def compute_kernel(self):
        ...
        if self.kernel == 'qSM':
            ...
            sigma_n = (params[-1])
            learnt_K = WN(self.acf_times,0, sigma_n)

            for o in range(order):
                sigma = (params[3*o+0])
                gamma = (params[3*o+1])
                mu = (params[3*o+2])
                learnt_K += Spec_Mix(self.acf_times,0, gamma, mu, sigma) 
            

            self.learnt_K = learnt_K
            self.learnt_K_label = f'Learnt {order}-comp Spectral Mix.'

    def compute_psd_ext(self, where_in_freq):
        step = where_in_freq[1]-where_in_freq[0]
        params = self.theta
        order = int(len(params)/3)
        learnt_S = np.zeros_like(where_in_freq)
        for o in range(order):
            learnt_S += params[3*o]**2 * np.exp(-(where_in_freq-params[3*o+2])**2/2/params[3*o+1]**2)
    ...
    def plot_kernel(self,title = None, f_true=None, K_true=None, label_true=None, benchmark = None, benchmark_label = None):
        ...
        c = plt.cm.Set1
        plt.figure(figsize=(12*0.9,4*0.9))       
        plt.plot(self.acf_times,self.acf_values, color = c(1) , markersize=10, label='Autocorrelation function')
        plt.plot(self.acf_times,self.learnt_K,color = c(0), linewidth=3, label= self.learnt_K_label)
        if f_true is not None:
            plt.plot(f_true, S_true,color = 'k', linewidth=3, label= label_true)
        if benchmark is not None:
            plt.plot(self.acf_times, benchmark, color = 'k', linewidth=3, label= benchmark_label)
        plt.title(title)
        plt.xlabel('time')
        plt.legend(loc = 0)
        #plt.ylim([2*min(self.acf_values),2*max(self.acf_values)])
        plt.tight_layout()
    ...
    def compute_acf2(self):
        ...
        #quantise
        lags_m = np.around(lags_m/np.max(lags_m), 3)*np.max(lags_m)
